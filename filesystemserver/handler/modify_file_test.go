...
// as actual tab, matching a tab-indented file content.
func TestModifyFile_ExactFindWithEscapedTab(t *testing.T) {
	dir := t.TempDir()
	filePath := filepath.Join(dir, "test.txt")
	// File contains real tabs
	originalContent := "before\tmatch\tafter"
	err := os.WriteFile(filePath, []byte(originalContent), 0644)
	require.NoError(t, err)
	...
	// find uses "\\t" (Go string = "\t") which should be interpreted as real tab
	request := mcp.CallToolRequest{}
	request.Params.Name = "modify_file"
	request.Params.Arguments = map[string]any{
		"path":            filePath,
...
// as actual tab when regex mode is enabled.
func TestModifyFile_RegexFindWithEscapedTab(t *testing.T) {
	...
	// find uses "\\t" in regex pattern to match tab indentation
	request := mcp.CallToolRequest{}
	request.Params.Name = "modify_file"
	request.Params.Arguments = map[string]any{
		"path":            filePath,
...
func TestModifyFile_ExactFindWithEscapedNewline(t *testing.T) {
	...
	// find uses "\\n" (Go string = "\n") which should be interpreted as real newline
	request := mcp.CallToolRequest{}
	request.Params.Name = "modify_file"
	request.Params.Arguments = map[string]any{
		"path":            filePath,
